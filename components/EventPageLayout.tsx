...
export default function EventPageLayout(props: EventPageProps) {
  ...
  const packageTiers = getDefaultPackageTiers({
    bronzeDesc,
    goldDesc,
    platinumDesc,
    bronzeBenefits,
    ...
    <>
      ...
      <div className={showModal ? 'blur-sm pointer-events-none select-none' : ''}>
        <div className="min-h-screen bg-black text-white overflow-x-hidden">
          ...
          <PackageCardsGrid
            tiers={packageTiers}
            onBookBronze={openBronze}
            onBookGold={openGold}
            onBookPlatinum={openPlatinum}
            bronzeLabel={bronzeLabel}
            goldLabel={goldLabel}
            platinumLabel={platinumLabel}
            maxWidth={extraPackageColumn ? 'max-w-7xl' : 'max-w-5xl'}
            insertAfterBronze={extraPackageColumn ? (
              <Reveal delay={0.08}>
                <div className="relative rounded-3xl overflow-hidden border border-sky-400/30 bg-gradient-to-br from-sky-500/10 via-black to-black p-6 md:p-7 h-full flex flex-col">
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    <div className="flex justify-center mb-4">
                      <span className="inline-flex items-center gap-2 bg-sky-500/20 text-sky-200 text-[11px] font-black tracking-widest uppercase px-4 py-1.5 rounded-full border border-sky-400/40">
                        {extraPackageColumn.badge}
                      </span>
                    </div>
