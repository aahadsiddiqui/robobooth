...
/* ════════════════════════════════════════════════════════════════
   VOGUE BOOTH — Custom Landing Page
   ════════════════════════════════════════════════════════════════ */
export default function VogueBooth() {
  ...
  return (
    <>
      <Head>
        <title>Vogue Booth Rental Toronto GTA | Magazine Cover Photo Booth | Robo Booth</title>
        <meta name="description" content="Step inside the GTA's most iconic photo booth. The Vogue Booth transforms your guests into magazine cover stars with custom overlays, studio lighting, and instant prints. Perfect for weddings, galas, and brand activations." />
        <meta name="keywords" content="vogue booth Toronto, magazine cover photo booth GTA, luxury photo booth Toronto, vogue photobooth rental, editorial photo booth, wedding photo booth Toronto" />
        <meta property="og:title" content="Vogue Booth Rental Toronto GTA | Magazine Cover Photo Booth | Robo Booth" />
        <meta property="og:description" content="Transform your guests into magazine cover stars. Custom Vogue-style overlays, studio lighting, and instant prints. Toronto & GTA." />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-center">
                <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
                  ...
                  <p className="text-white/80 text-sm md:text-base lg:text-lg leading-relaxed mb-5 max-w-xl">
                    Step inside the illuminated Vogue Booth — a 7'6" × 6'6" enclosed studio experience with custom editorial overlays, studio-quality lighting, and instant prints that guests keep forever.{' '}
                    <span className="text-white font-semibold">The most talked-about booth at every event it appears at.</span>
                  </p>
            ...
            <div className="max-w-2xl mx-auto">
              <Reveal>
                <div className="rounded-2xl overflow-hidden border border-white/10 bg-black flex items-center justify-center">
                  <img src="/images/voguebooth2.png" alt="Vogue Booth illuminated" className="w-full h-auto object-contain" style={{ display: 'block', maxHeight: '60vh' }} loading="lazy" />
                </div>
              </Reveal>
            </div>
          </section>
          ...
          <section className="py-8 md:py-10 px-4 border-t border-white/5">
            <div className="max-w-5xl mx-auto">
              <Reveal className="text-center mb-6">
                ...
                <p className="text-white/50 text-xs md:text-sm">The booth that makes every guest feel like the main character</p>
              </Reveal>
          ...
          <section className="py-8 md:py-10 px-4 border-t border-white/5">
            <div className="max-w-5xl mx-auto">
              <Reveal className="text-center mb-6">
                ...
                <p className="text-white/50 text-xs md:text-sm">Every detail tailored to match your event, brand, or theme</p>
              </Reveal>
          ...
          <section className="py-10 md:py-14 px-4 border-t border-white/5">
            <Reveal className="max-w-3xl mx-auto text-center">
              <h2 className="text-xl md:text-2xl lg:text-4xl font-black mb-2 md:mb-3">
                Give Your Guests the Most <span className="text-[#fce4a6]">Unforgettable Photo Experience.</span>
              </h2>
              <p className="text-white/60 text-xs md:text-sm lg:text-base mb-5 max-w-lg mx-auto">
                The Vogue Booth is a statement piece that transforms any event into an editorial moment. Limited dates available — book yours now.
              </p>
            ...
            <motion.div initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 60 }}
              ...
              {packageType === 'bronze' && (
                <div className="bg-white/90 border border-black/10 rounded-xl px-4 py-2.5 mb-3 flex items-center justify-center gap-2 flex-wrap">
                  ...
                  <span className="text-black/60 text-[10px]">Vogue Booth Only</span>
                </div>
              )}
              {packageType === 'gold' && (
              ...
              <h2 className="text-lg md:text-2xl font-black text-black mb-1 text-center">
                {packageType === 'gold' ? 'Book Gold Package' : packageType === 'bronze' ? 'Book Bronze Package' : packageType === 'platinum' ? 'Book Platinum Package' : 'Reserve the Vogue Booth'}
              </h2>
...
/* ─── DATA ─── */
const howItWorks = [
  { title: 'You Book', desc: 'Tell us your event date, venue, and vision. We confirm availability quickly and design a custom editorial overlay just for your event.' },
  { title: 'We Set Up', desc: 'Our team arrives early and sets up the illuminated Vogue Booth in under an hour — fully assembled, lit, and ready before the first guest walks in.' },
  { title: 'Your Guests Are Wow\'d', desc: 'Guests step inside, strike their pose, and walk away with a magazine cover keepsake in hand. Every single one feels like a cover star.' },
]

const whyGuestsLove = [
  { icon: <FiCamera className="w-5 h-5 md:w-6 md:h-6" />, title: 'Magazine Cover Moment', desc: 'Custom editorial overlays transform every photo into a personalized Vogue-style magazine cover — a keepsake guests actually want to display.' },
  { icon: <FiZap className="w-5 h-5 md:w-6 md:h-6" />, title: 'Studio-Quality Lighting', desc: 'Built-in professional LED lighting inside the booth ensures every guest looks polished, bright, and camera-ready — no bad photos.' },
  { icon: <FiImage className="w-5 h-5 md:w-6 md:h-6" />, title: 'Instant Prints & Digital Delivery', desc: 'Prints in hand and photos on their phone within seconds. Guests share, post, and tag before they\'ve even left the booth.' },
  { icon: <FiUsers className="w-5 h-5 md:w-6 md:h-6" />, title: 'A Statement Piece at Any Event', desc: 'At 7\'6" tall with full illumination, the Vogue Booth commands attention across the room. It becomes the focal point of every event it\'s at.' },
  { icon: <FiStar className="w-5 h-5 md:w-6 md:h-6" />, title: 'Fully Branded Experience', desc: 'Custom overlay text, colors, fonts, and event branding on every print. Perfect for weddings, galas, brand activations, and luxury events.' },
  { icon: <FiShield className="w-5 h-5 md:w-6 md:h-6" />, title: 'White-Glove Service', desc: 'A dedicated attendant manages the entire experience — setup, guest guidance, and teardown. You don\'t lift a finger.' },
]

const customizations = [
  { icon: <FiImage className="w-5 h-5" />, title: 'Custom Magazine Cover Overlay', desc: 'We design a bespoke editorial overlay for your event — custom masthead text, cover lines, fonts, and colors that make every print feel one-of-a-kind.' },
  { icon: <FiStar className="w-5 h-5" />, title: 'Event Branding & Theme Matching', desc: 'Your logo, event name, date, hashtag, and color palette integrated into the booth experience. Perfect for weddings, galas, and branded activations.' },
  { icon: <FiShield className="w-5 h-5" />, title: 'Full White-Glove Service', desc: 'Delivery, setup, professional operation, and teardown all handled by our team. You focus on your guests — we make you look exceptional.' },
]

const testimonials = [
  { name: 'Sophia R.', role: 'Bride, Wedding Reception', text: 'The Vogue Booth was the most talked-about thing at our wedding. Every single guest went through it at least twice. The prints are absolutely stunning — mine is already framed.' },
  { name: 'Marcus T.', role: 'Brand Activation Manager', text: 'We needed something that felt premium and editorial for our launch event. The Vogue Booth delivered exactly that. The custom cover overlay with our brand was flawless.' },
  { name: 'Danielle K.', role: 'Gala Event Organizer', text: 'Our guests couldn\'t stop talking about it. The illuminated booth looks incredible in photos and the prints feel like actual magazine covers. Will absolutely book again.' },
]

const faqs = [
  { question: 'What are the dimensions of the Vogue Booth?', answer: 'The Vogue Booth measures 7 feet 6 inches tall, 6 feet 6 inches in depth, and 6 feet 6 inches in width. It comfortably fits 1–4 guests at a time and requires a space slightly larger than the booth itself for access.' },
  { question: 'How much space does the Vogue Booth need at the venue?', answer: 'We recommend a minimum 10×10 ft area for comfortable guest flow and operation. The booth is self-contained and requires a standard power outlet — no WiFi needed.' },
  { question: 'Can the overlay be fully customized for our event?', answer: 'Absolutely. We design a custom editorial overlay for every booking — including custom masthead text, cover lines, fonts, colors, logos, and event details. We send a proof for your approval before the event.' },
  { question: 'What\'s included in the rental?', answer: 'Everything — the illuminated Vogue Booth, custom editorial overlay design, on-site professional attendant, instant prints, digital photo delivery, setup, and teardown. All you need to do is show up.' },
  { question: 'How quickly do guests receive their photos?', answer: 'Instantly. Digital copies are delivered to guests\' phones via QR code within seconds of the photo being taken. Prints are produced on-site and in guests\' hands within moments.' },
  { question: 'What types of events is the Vogue Booth best for?', answer: 'The Vogue Booth is perfect for weddings, galas, brand activations, product launches, holiday parties, birthdays, engagements, and any event where you want a premium, Instagram-worthy experience.' },
  { question: 'How far in advance should I book?', answer: 'We recommend booking at least 4–6 weeks in advance to allow time for custom overlay design and approval. Peak season weekends fill quickly — contact us now to check your date.' },
]
